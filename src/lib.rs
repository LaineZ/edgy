...
where
    ...
    C: PixelColor,
{
    pub fn new<F>(ui: &mut UiContext<D, C>, direction: StackLayoutDirection, mut f: F) -> Self
    where
        F: FnMut(&mut dyn FnMut(Box<dyn Widget<D, C> + 'a>)),
    {
        ...
        let mut add_widget = |widget: Box<dyn Widget<D, C> + 'a>| {
            let widget_size = widget.size();
            let current_position = ui.bounds.top_left;
            let remaining_size = ui.bounds.size;

            if (direction == StackLayoutDirection::Vertical && widget_size.height > remaining_size.height)
                || (direction == StackLayoutDirection::Horizontal && widget_size.width > remaining_size.width)
            {
                return; 
            }

            let child_ui = UiContext {
                draw_target: ui.draw_target,
                bounds: Rectangle::new(current_position, widget_size),
            };
            ...
            match direction {
                StackLayoutDirection::Vertical => {
                    ui.bounds.top_left.y += widget_size.height as i32;
                    ui.bounds.size.height -= widget_size.height;
                }
                StackLayoutDirection::Horizontal => {
                    ui.bounds.top_left.x += widget_size.width as i32;
                    ui.bounds.size.width -= widget_size.width;
                }
            }
        };
    ...
    pub fn draw(&mut self, ui: &mut UiContext<D, C>) {
        ...
        for widget in &mut self.widgets {
            ...
            if (self.direction == StackLayoutDirection::Vertical && widget_size.height > remaining_size.height)
                || (self.direction == StackLayoutDirection::Horizontal && widget_size.width > remaining_size.width)
            {
                continue;
            }
